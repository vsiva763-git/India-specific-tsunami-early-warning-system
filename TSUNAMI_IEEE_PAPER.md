...
**Keywords:** Multi-modal deep learning, CNN-LSTM hybrid architecture, ocean wave disaster prediction, tsunami early warning, IoT edge computing, Arduino, ESP8266, seismic monitoring, real-time alert dissemination, coastal safety.
...
India-specific tsunami warning research has focused on bathymetric and geographic considerations. Studies analyzed tsunami propagation patterns in the Indian Ocean [11], while others studied building vulnerability assessment along Indian coastlines [12]. Recent work developed AI-based tsunami risk assessment systems specific to Indian geographies [13]. The proposed multi-modal CNN-LSTM hybrid architecture builds upon these foundations by integrating spatial feature extraction through convolutional networks with temporal sequence modeling through recurrent networks, combining real-time multi-modal data processing with distributed IoT alert dissemination specifically optimized for Indian coastal deployment.
...
The cloud layer continuously monitors the Indian Ocean region through integration with multiple heterogeneous data sources including USGS Earthquake API, NOAA Tides, NOAA NDBC Buoys, and INCOIS (Indian National Centre for Ocean Information Services) advisories. The multi-modal CNN-LSTM hybrid architecture processes this diverse seismic, oceanographic, and bathymetric data through convolutional layers for spatial feature extraction and LSTM layers for temporal pattern recognition to generate real-time ocean wave disaster risk assessments. India-specific filtering algorithms evaluate epicenter location, distance to Indian coastlines, propagation direction, and bathymetric features to determine whether detected seismic events pose credible threats to Indian territories.
